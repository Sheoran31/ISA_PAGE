"""
Zerodha Alert Sender - Send alerts and handle order execution with manual confirmation
"""
...
class ZerodhaAlertSender:
    """Send alerts to Zerodha and manage order execution with manual confirmation"""

    def __init__(self):
        """Initialize Zerodha alert sender"""
        self.zerodha_client = get_zerodha_client()
        self.pending_orders = {}  # Track pending confirmations
        logger.info("ZerodhaAlertSender initialized")
    ...
    def confirm_and_execute(self, order_id: str, quantity: int = 1, order_type: str = "MARKET",
        ...
        try:
            ...
            alert_data['zerodha_order_id'] = order_response.get('order_id')
            alert_data['execution_quantity'] = quantity

            logger.info(f"\n✅ ORDER EXECUTED SUCCESSFULLY")
            logger.info(f"{'='*70}")
            logger.info(f"Zerodha Order ID: {order_response.get('order_id')}")
            logger.info(f"Symbol: {symbol}")
            logger.info(f"Side: {side}")
            logger.info(f"Quantity: {quantity}")
            ...
            return {
                ...
                'side': side,
                'quantity': quantity,
                'zerodha_order_id': order_response.get('order_id'),
                'order_type': order_type,
...
# Global instance
_zerodha_sender_instance = None


def get_zerodha_sender() -> ZerodhaAlertSender:
    """Get or create global ZerodhaAlertSender instance"""
    global _zerodha_sender_instance
    if _zerodha_sender_instance is None:
        _zerodha_sender_instance = ZerodhaAlertSender()
    return _zerodha_sender_instance
